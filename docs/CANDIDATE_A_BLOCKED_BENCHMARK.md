...
Every Candidate A case atomically writes and verifies `<server_dir>/plugins/bstats/config.toml` with `enabled = false` before BDS startup. The canonical file bytes, relative path, size, SHA-256, and parsed false value are retained in case protocol evidence and copied into the allowlisted evidence package; missing, malformed, tampered, escaped, or unallowlisted bStats evidence invalidates the case.
