...
class Pr49NoShimWindowsRunnerTest(unittest.TestCase):
    def test_validate_no_shim_run_requires_exact_lab_carrier_and_workflow(self) -> None:
        lab_sha = "a" * 40
        run = {
            "id": 123,
            "head_sha": lab_sha,
        ...
        runner._validate_no_shim_run(run, expected_lab_sha=lab_sha, expected_run_id=123)

        bad = dict(run, name="Build")
        with self.assertRaisesRegex(RuntimeError, "workflow mismatch"):
            runner._validate_no_shim_run(bad, expected_lab_sha=lab_sha, expected_run_id=123)

        wrong_carrier = dict(run, head_sha="b" * 40)
        with self.assertRaisesRegex(RuntimeError, "workflow SHA mismatch"):
            runner._validate_no_shim_run(wrong_carrier, expected_lab_sha=lab_sha, expected_run_id=123)
    ...
    def _write_verified_payload(self, root: Path, sha: str) -> None:
        ...
        (root / "spark-candidate-sha.txt").write_text(sha, encoding="ascii")
        (root / "spark-source-repository.txt").write_text(runner.SPARK_REPOSITORY, encoding="ascii")

    def test_assert_no_shim_payload_accepts_verified_single_dll_payload(self) -> None:
        sha = "c" * 40
        with tempfile.TemporaryDirectory() as tmp, mock.patch.dict(
            os.environ, {"EXPECTED_SPARK_SHA": sha}, clear=False
        ):
            root = Path(tmp)
            self._write_verified_payload(root, sha)
    ...
    def test_assert_no_shim_payload_rejects_wrong_embedded_candidate(self) -> None:
        sha = "e" * 40
        with tempfile.TemporaryDirectory() as tmp, mock.patch.dict(
            os.environ, {"EXPECTED_SPARK_SHA": sha}, clear=False
        ):
            root = Path(tmp)
            self._write_verified_payload(root, "f" * 40)
            with self.assertRaisesRegex(RuntimeError, "embedded Spark candidate SHA mismatch"):
                runner._assert_no_shim_payload(root)

    def test_resolve_exact_no_shim_spark_pins_lab_run_and_candidate_artifact(self) -> None:
        spark_sha = "1" * 40
        lab_sha = "2" * 40
        run = {
            "id": 789,
        ...
        artifact = {
            "id": 987,
            "name": f"spark-windows-no-shim-{spark_sha}",
            "expired": False,
        }
        requested: list[str] = []
        ...
        self.assertTrue(all(f"/repos/{runner.LAB_REPOSITORY}/" in path for path in requested))
